...
def findMaxBytesInFlight(pcapfile):
    ...
    flow = readHandShake(packets[:3])

    for p in packets[3:]:
        if TCP in p:
            if isFlowEgress(p, flow):
                seqNum = p[TCP].seq
                payloadLen = len(p[TCP].payload)

                # Update highest sequence number seen
                if seqNum + payloadLen > flow.highestSeqNum:
                    flow.highestSeqNum = seqNum + payloadLen
                    flow.pktLenOfHighestSeqNumPacket = payloadLen
                ...
                if ackNum > flow.ackNumReceived:
                    flow.ackNumReceived = ackNum

                bytesInFlight = flow.highestSeqNum - flow.ackNumReceived
