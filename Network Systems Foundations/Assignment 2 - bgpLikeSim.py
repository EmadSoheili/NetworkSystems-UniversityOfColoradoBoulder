...
class Router:
    ...
    def next_hop(self, ipaddr):
        ...
        for pfx, routes in self.rib.items():
            ...
            pfx_len = int(pfx_len)
            pfx_bin = self.convertToBinaryString(pfx_ip)[:pfx_len]
            if ip_bin.startswith(pfx_bin):
                if pfx_len > longest_match:
                    longest_match = pfx_len
                    best_route = min(routes, key=lambda route: len(route.path))
